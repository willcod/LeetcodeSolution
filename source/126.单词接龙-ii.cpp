...
class Solution {
   public:
    vector<vector<string>> findLadders(string beginWord, string endWord,
        ...
        unordered_set<string> dict(wordList.begin(), wordList.end()), head,
            tail;
   ...
   private:
    bool searchLadders(unordered_set<string>& head, unordered_set<string>& tail,
            ...
            for (int i = 0; i < word.size(); i++) {
                ...
                for (int j = 0; j < 26; j++) {
                    ...
                    if (tail.find(word) != tail.end()) {
                        ...
                        flip ? children[word].push_back(temp)
                             : children[temp].push_back(word);
